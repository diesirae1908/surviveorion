...
- `altool --upload-app` delivery/build `a0f45ac0-0eff-49d4-ab26-f1b2b7654b71`. Processing `VALID`. `usesNonExemptEncryption:false`. Internal group `1c4f517b-f1cd-4f8b-a2a7-c770b0e09dfe`: `IN_BETA_TESTING`. Attached to App Store version 1.0 `cc936b94-bb8b-4d22-a4bc-183ee4fb1809` (state `PREPARE_FOR_SUBMISSION`). Not submitted for review.
- Follow-up for Lucas: confirm native Google sign-in E2E on device (PKCE flow); eyeball Delete account footer placement when signed in.
...
- **Paywall (3.1.2):** `PremiumSheet.swift` Terms `Link` + auto-renew disclosure under price cards.
- **iPhone-only:** `project.pbxproj` `TARGETED_DEVICE_FAMILY = "1"` (Debug + Release).
- **Native Google:** `ASWebAuthenticationSession` in `NativeAuth.swift`; `GOOGLE_IOS_CLIENT_ID` → `/api/config` `googleIosClientId` + `verifyGoogleToken` aud accept in `server/index.mjs`; `Info.plist` URL scheme `com.googleusercontent.apps.846475365993-k8im01ghtevbij90idmk50384mdfutc0`. Render env already set on `surviveorion`.
- **Patrol calendar mutators:** `server/mutator-schedule.json` ships in Docker; `export-mutator-schedule.ts` writes both server + iOS copies.
- **TF9 fixes kept from main:** calendar redesign, game-over pitch/music, settings cleanup. **Our additions:** `LobbyMusic` sting ducking (bed silent under patrol-complete sting), `ClipStore.photosNeverAsked()`, tilt tribute copy removed from pause menu (`ui.ts`).
- Follow-ups: attach build 10 to ASC version 1.0 after upload; Lucas eyeballs Terms/Privacy row on paywall; confirm native Google E2E on device once deploy serves `googleIosClientId`.

## 2026-09-13 night PT: TestFlight feedback triage (builds 2-9), four clear-bug fixes (uncommitted)

- Read all 30 `betaFeedbackScreenshotSubmissions` (0 crash submissions) for app id `6811113450` via the App Store Connect API, downloaded every screenshot to `~/Documents/Sam/tmp/orion-aso/tf-feedback/`, mapped each to its build via the `build` relationship + included `builds` version, and cross-referenced this `JOURNAL.md`'s 2026-09-11 to 2026-09-13 entries. Full table: `~/Documents/Sam/tmp/orion-aso/tf-feedback/SUMMARY.md`. Most items from builds 2-6 (main menu redesign, Gold Patrol rename/tagline, restore-purchase copy, tilt onboarding copy, board time/platform columns) were already fixed by later builds already in this journal; verified each against the actual screenshots and current code rather than assuming from dates alone. Four build-9 items were still open and are clear, minimal bug fixes, not product decisions, so implemented here:
- **Patrol Calendar always shows "CLASSIC" for every day once signed in** (build 9, "and here it is broken btw I think, it is all classic"). Root cause: `server/index.mjs`'s `GET /api/patrol-mutators` (`loadMutatorSchedule()`) looks for `server/mutator-schedule.json` first, then falls back to a relative `../ios/App/App/Resources/mutator-schedule.json` path — but `server/mutator-schedule.json` never existed, and the Docker image (`Dockerfile`: `COPY server ./server`, `COPY --from=build /app/dist ./dist`) never contains an `ios/` directory at all, so on live/staging the schedule always loads as `{}` and every date falls back to the hardcoded `"CLASSIC"` placeholder. The native app's own bundled `MutatorCatalog` (used for guests / not-signed-in) reads its own copy inside the app bundle, so it never showed the bug, masking it for guest testing. Fix: `scripts/export-mutator-schedule.ts` now writes to both `ios/App/App/Resources/mutator-schedule.json` and the new `server/mutator-schedule.json` (which ships in the Docker image via the existing `COPY server ./server`); ran it, both files are identical and cover through 2027-12-31. Needs a server deploy to actually fix the live symptom; the two JSON files must stay in sync going forward by re-running the export script after any mutator schedule change.
- **PATROL COMPLETE popup sting doubles with the menu music bed** (build 9, "Remove the music for that pop up, it's double with the menu music"). `LobbyMusic.swift`'s `playSting()` played the one-shot `patrol-complete.mp3` while the looping `empire-of-the-stars.mp3` bed kept going underneath, by design per the existing comment. Now `playSting()` ducks the bed to volume 0 first (remembering its prior volume) and an `AVAudioPlayerDelegate` callback restores it when the sting finishes; `play()`/`pause()`/`resumeIfNeeded()` also unduck defensively so a ducked bed can never get stuck silent.
- **No Photos permission prompt when opening Orion's Settings** (build 9, "I can't give access to my photo, there is no photo access when I open the setting for Orion"). `SettingsView.swift`'s CREW TOOLS "Photos access" row jumped straight to the system Settings app whenever status wasn't `.granted`, including on a fresh install where iOS has never shown the permission dialog — so there is no Orion entry in system Settings to toggle, a dead end. Added `ClipStore.photosNeverAsked()` (raw `PHPhotoLibrary` status is `.notDetermined`); the row now calls `ClipStore.requestPhotos()` (the same async API `GameOverView.saveClip()` already uses) when never asked, falling back to system Settings only if that still isn't granted or if iOS previously denied it.
- **Copy**: removed the residual "A tribute to Tilt to Live." clause from the in-run pause-menu tilt hint in `src/ui.ts` (build 3, "Remove the 'tribute to til to live' but keep the end of the sentence"). The pre-game "Choose Your Controls" sheet had already dropped this phrase by build 4; this was the same phrase surviving in a second, separate screen (the in-game flight manual / pause menu), left inconsistent until now.
- Did not touch StoreKit, `/api/me/premium`, `apple-iap.mjs`, or the CORS allowlist. Did not revert the pre-existing uncommitted paywall Terms/disclosure, iPhone-only target, or Google Sign-in work above.
- Verify: `npm test` green (277 PASS, exit 0). `npm run build` green (`tsc --noEmit` + `vite build`). `xcodebuild ... -destination 'platform=iOS Simulator,name=iPhone 17' build` → **BUILD SUCCEEDED**. Installed via `xcrun simctl install`/`launch` on simulator `46C65C1F-E94F-4E27-AF8B-80F5F4659E62` (the `Claude Code iOS Simulator` MCP tool's `attach`/`launch` refused with "Xcode is installed but not selected" even though `xcode-select -p` and a plain `xcodebuild` both resolve Xcode correctly in this shell; did not run the `sudo xcode-select -s ...` it asked for, that needs Lucas's password and is a system-settings change outside this session's scope). Screenshots via `xcrun simctl io screenshot`: `~/Documents/Sam/tmp/orion-aso/tf-feedback/verify-01-premium-sheet.png` (Gold Patrol sheet, `-QAPlay premium -QAPremium`, confirms the already-fixed tagline/disclosure/restore copy still renders, no regression) and `verify-02-settings-crew.png` (Settings CREW TOOLS, `-QACrew -QAPlay crew`, confirms the edited Photos access row still renders correctly). Could not visually confirm the calendar fix in-simulator: the app talks to the live/staging server, not this local checkout, so the "CLASSIC" symptom only actually clears once `server/mutator-schedule.json` ships in a deploy. Could not interactively tap-test the Photos permission prompt without the blocked simulator control tool; verified by code review against the existing, working `GameOverView.saveClip()` pattern instead.
- Several other build-9 items are real but are product decisions or need repro steps Lucas didn't give (mutator pairing rules, calendar redesign, premium upsell placement, sharing-with-song feature, duplicate Home/Settings nav rows, a narrow/black-void layout glitch on rehearsal launch, "stuck on screen" with no further detail, "main menu bugged when quitting" from build 3): left alone, listed as open questions in the SUMMARY and in Sam's report.

## 2026-09-13 evening PT: Paywall Terms link + auto-renew disclosure (guideline 3.1.2, uncommitted)

- Isolated `.worktrees/fix-paywall-terms` from `origin/main` (`8720412`). Dirty parent checkout untouched. Did not reset `origin/dev`. Not committed, per dispatch brief (Sam reviews and owns the push).
- Compliance gap from `~/Documents/Sam/tmp/orion-aso/compliance-sheet.md` section 4: `PremiumSheet.swift` had a working Privacy `Link` but "Terms" was plain, untappable `Text`, and no auto-renewal/cancellation disclosure existed anywhere in the sheet.
- `ios/App/App/PremiumSheet.swift`: "Terms" is now a `Link` to `https://surviveorion.com/terms.html`, styled identically to the Privacy link (`OrionColor.dust`, no opacity). Added a 3-sentence disclosure paragraph under the price cards ("Gold Patrol renews automatically at $X per month or $Y per year until cancelled. Payment goes to your Apple Account at confirmation. Cancel anytime in Settings, at least 24 hours before the period ends."), same muted style as the existing "Use this if you paid on another device..." line. Prices build from `model.store.monthly`/`.yearly` `displayPrice` (StoreKit) when the catalog is loaded, falling back to the sheet's existing hardcoded "$1.99"/"$14.99" strings when it isn't (confirmed in the simulator screenshot, where the StoreKit catalog is unavailable and the fallback rendered).
- Skipped the optional hygiene item (unguarded `-QA*` launch-argument branches in `OrionApp.swift`/`WebPlay.swift`): neither file already uses the `#if DEBUG` pattern locally (it only exists in `AppModel.swift`), and the `OrionApp.swift` QA block is the whole `onAppear` QA-arg switch, not a few lines. Left it alone per the brief's "if in doubt, skip" instruction.
- Verify: `npm ci` + `npm run build` green (dist rebuilt, needed for the `WKWebView` bundle). `xcodebuild ... -destination 'platform=iOS Simulator,name=iPhone 17' build` succeeded. Installed and launched on simulator `46C65C1F-E94F-4E27-AF8B-80F5F4659E62` with `-QAPlay premium`. Screenshot at `~/Documents/Sam/tmp/orion-aso/paywall-fix.png` confirms the disclosure paragraph is visible without scrolling. Did not confirm the Terms/Privacy row visually (it sits below the fold in the sheet's `ScrollView` and no UI-automation tool was available in this session to scroll the simulator); confirmed by code review that both links share the same `Link` construct and style.
- Follow-up for Sam: review, decide on committing/pushing, and manually scroll the paywall once to eyeball the Terms/Privacy row before submission.
- Also, per Sam: iPad layout is broken (phone-width block, most of the 13" screen black), so 1.0 ships iPhone only. `ios/App/App.xcodeproj/project.pbxproj` both `TARGETED_DEVICE_FAMILY = "1,2"` (Debug + Release) changed to `"1"`. Checked `Info.plist` and `ios/debug.xcconfig` for other iPad-only keys/settings: only `UISupportedInterfaceOrientations~ipad` exists and is inert once iPad is excluded, left as instructed. Rebuilt for iPhone 17 simulator: `BUILD SUCCEEDED`.
- Also, per Sam: Google sign-in was failing in TestFlight because `GoogleSignInController` (`ios/App/App/NativeAuth.swift`) ran the OAuth flow in an embedded `WKWebView`, and Google returns 403 disallowed_useragent for embedded WebViews. Replaced it with `ASWebAuthenticationSession` (`prefersEphemeralWebBrowserSession = false` so the device's existing Google session is reused), same accounts.google.com authorization URL, same `client_id` from `/api/config`, same `id_token` parsing, same downstream call into `/api/auth/google` (`AppModel.signInWithGoogle` / `APIClient` untouched, no server change). Sign in with Apple (`AppleSignInCoordinator` and below) untouched. **First pass used a custom-scheme redirect on the existing Web client, corrected below**: that fails, Google's Web-type OAuth client rejects non-https redirect URIs outright (redirect_uri_mismatch), it was never going to work.
- **Correction (still uncommitted, same session)**: completed it the standard way instead. `server/index.mjs`: added optional `GOOGLE_IOS_CLIENT_ID` env, exposed as `googleIosClientId` in `GET /api/config` (`null` when unset), and `verifyGoogleToken` now accepts `aud` equal to either the existing web `GOOGLE_CLIENT_ID` or `GOOGLE_IOS_CLIENT_ID` when set. Existing web flow (browser sign-in) byte-for-byte identical otherwise. `ios/App/App/APIClient.swift`: `AppConfig` gained `googleIosClientId: String?`. `ios/App/App/SettingsView.swift`: reads it alongside `googleClientId` and passes it into `GoogleSignInHost`. `ios/App/App/NativeAuth.swift`: `GoogleSignInController` now prefers `iosClientId` when set, falling back to the web `clientId` otherwise (current behavior, will still fail until the iOS client exists — that's expected, not a regression). The OAuth `redirect_uri` and `ASWebAuthenticationSession`'s `callbackURLScheme` are both derived at runtime from whichever client id is in play, using Google's standard "iOS" client redirect: reverse `<number>.apps.googleusercontent.com` into `com.googleusercontent.apps.<number>`, then `:/oauth2redirect`. Removed the `com.surviveorion.app://oauth2redirect` custom scheme and its `Info.plist` entry from the first pass (no longer used); replaced it with a `CFBundleURLTypes` entry for `com.googleusercontent.apps.PLACEHOLDER`, commented in the plist that PLACEHOLDER must become the real client number once the iOS OAuth client exists.
- Searched `scripts/test-*.mjs` for an existing server-auth test pattern to extend for Google (per the brief). No hits: the only "google" matches are in `test-clip-inbox.mjs`, which test the unrelated `CLIP_INBOX_GOOGLE_SUB` allowlist, not `/api/config` or `/api/auth/google`. No existing pattern to follow, so skipped adding a test rather than inventing a new harness style for one route.
- Verify: `npm test` green, all 289 assertions PASS, exit 0 (the 4 case-insensitive "fail" hits in the log are substrings of passing test names: "fail-closed", "fails closed", "load failure"). Rebuilt for iPhone 17 simulator: `BUILD SUCCEEDED`.
- **Lucas's checklist before build 10 / native Google sign-in works**:
  1. Google Cloud console: create a new OAuth client, type **iOS**, bundle id `com.surviveorion.app`, App Store id `6811113450`, team `4R88D2NKUC`.
  2. Take that client's id (`<number>-<hash>.apps.googleusercontent.com`) and put the `<number>-<hash>` piece into the `Info.plist` `CFBundleURLTypes` entry, replacing `PLACEHOLDER` in `com.googleusercontent.apps.PLACEHOLDER`.
  3. Set `GOOGLE_IOS_CLIENT_ID` (the full client id string) on Render service `surviveorion`.
  4. Redeploy the server before build 10 is tested, so `/api/config` actually serves `googleIosClientId`.
- Not runtime-tested end to end (needs all four checklist items first, then a device/TestFlight build; the custom-scheme callback isn't meaningfully exercisable on a local Debug simulator build signed for local run).
...
- **Sunday pairing rule (Lucas: "choose your best choice"):** `HOLD_ONE_PAIR_RULE_FROM = 2026-09-14`. From that date a hold-one pickup day (GOLD DASH, RAM RAID) never pairs with an evolution-shaping "assembly-kind" day (TITANFALL, MENAGERIE, forced-creature days). Dated, so today and every flown Sunday keep their pair; snapshot through Dec 31 unchanged (no such pair was scheduled), bundled schedule re-exported (one 2027 Sunday moved).
- **Not changed, for Lucas:** "Data saved across builds": Keychain + UserDefaults persist across TestFlight installs and sessions last 30 days; could not reproduce, need what was lost.
